...
export function createHal9000Avatar(THREE, currentState = {}) {
  ...
  const materials = {
    panel: new THREE.MeshPhysicalMaterial({
      ...
      metalness: clamp(currentState.metalness ?? 0.78, 0, 1),
      roughness: clamp(currentState.roughness ?? 0.28, 0, 1),
      clearcoat: clamp(currentState.clearcoat ?? 0.58, 0, 1),
      clearcoatRoughness: clamp(currentState.clearcoatRoughness ?? 0.24, 0, 1),
